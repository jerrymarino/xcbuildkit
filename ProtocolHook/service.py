...
def handle_obj(obj, unpacker):
	...
	# Note: the result here is different depending on how we create the session
        ...
        ctx.writeb(b"\xbdTRANSFER_SESSION_PIF_RESPONSE\x91\x90\x03\x00\x00\x00")
        ctx.dump()
        ...
        #session_id = info[1]
        log("CREATE_BUILD Info")
        ctx.writeb(b"\x00\x00\x00\x00\x00\x00\x00\x18\x00\x00\x00\xadBUILD_CREATED\x91\xd3\x00\x00\x00\x00\x00\x00\x00\x00\x07")
        ctx.dump()
